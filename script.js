...
function renderKPIs(data) {
  ...
  const _ld = allData.map(r => r.date).filter(Boolean); const _ldt = _ld.length ? new Date(Math.max(..._ld)).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'}) : '';
